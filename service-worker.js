...
const SW_BUILD = '2.1.3'; // <-- manter sincronizado com a 1ª linha de sw-version.txt

// Caminhos relativos à pasta do service-worker.js. Assim o site funciona na raiz
// do domínio e também em subpastas (ex: usuario.github.io/projeto/).
const ROOT = new URL('./', self.location.href);
const VERSION_URL = new URL('sw-version.txt', ROOT);
...
const FALLBACK_FILES = [ROOT.href, new URL('index.html', ROOT).href];

// Arquivos grandes ou de mídia passam direto pela rede. O cache não guarda
// respostas parciais (Range), que áudio e vídeo usam.
const NETWORK_ONLY = /\.(pdf|mp3|mp4|webm|ogg)$/i;
...
async function fetchVersionManifest() {
  ...
  if (!response.ok) {
    throw new Error(`Não foi possível buscar ${VERSION_URL.pathname}: ${response.status}`);
  }
  ...
  // Cada linha é um caminho relativo à raiz do site (ex: css/styles.css)
  const files = lines.slice(1).map((path) => new URL(path, ROOT).href);
...
self.addEventListener('fetch', (event) => {
  ...
  // Áudio, vídeo e PDF não passam pelo cache (ver NETWORK_ONLY)
  if (request.headers.has('range') || NETWORK_ONLY.test(url.pathname)) return;

  // sw-version.txt NUNCA vem do cache — sempre rede, é ele quem dita a verdade
  if (url.pathname === VERSION_URL.pathname) {
